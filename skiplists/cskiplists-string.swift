...
public class CSkipList: SequenceType {
    ...
    func deleteMatchedValues(s: UnsafeMutablePointer<C_SkipListSearch>) {
        ...
        let _: SkipListValue = Unmanaged.fromOpaque(COpaquePointer(p)).takeRetainedValue()
    ...
    public func delete(key: String, value: String) -> Bool {
        let s = newSkipListSearch(list);
        guard s != nil else { return false }
        defer { destroySkipListSearch(s); }
    ...
    public func generate() -> AnyGenerator<(String, String)> {
        ...
        return AnyGenerator<(String, String)> {
            ...
            while state == .Traversing {
                ...
                let tmp: SkipListValue = Unmanaged.fromOpaque(COpaquePointer(p)).takeUnretainedValue()
                v = tmp
            }
            return nil
        }
